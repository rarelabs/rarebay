...
const Swap = (props: { rootClassName: any; imageAlt: string; button7: any; button: any; button6: any; button5: any; button8: any; button72: any; button4: any; }) => {
...
if(!clicked){
  ...
  const transaction = prepareContractCall({
    ...
    method: "swapTokens",
    params: [inputAmountWei, inputToken?.address, outputToken?.address, address],
  });
...
return (
    <>
      ...
      <div className={`swap-swap ${props.rootClassName} `}>
        <div className="swap-container1">
         ...
         {token1 &&  <div className="swap-container4">
            ...
            <span className="swap-text16">{currentChain && token1?.symbol != 'CORE' ? formatNumber(Number(toTokens(balance1, 18))?.toFixed(5)):formatNumber(native?.displayValue)} {token1?.symbol}</span>
          </div>}
        </div>
        <div 
        className={`swap-swapbutton`}>
          <div className={`swap-container5 ${clicked ? "clicked" : ""}`}
          ...
          >
            <svg
              ...
              height="24"
              viewBox="0 0 24 24"
              className="swap-icon1"
            >
              <path
                d="M16 17.01V11c0-.55-.45-1-1-1s-1 .45-1 1v6.01h-1.79c-.45 0-.67.54-.35.85l2.79 2.78c.2.19.51.19.71 0l2.79-2.78c.32-.31.09-.85-.35-.85zM8.65 3.35L5.86 6.14c-.32.31-.1.85.35.85H8V13c0 .55.45 1 1 1s1-.45 1-1V6.99h1.79c.45 0 .67-.54.35-.85L9.35 3.35a.5.5 0 0 0-.7 0"
                fill="currentColor"
              ></path>
            </svg>
          </div>
        ...
        <div className="swap-container6">
          ...
          />
          {
            ...
            <span className="swap-text18">{token2?.symbol != 'CORE' ? formatNumber(Number(toTokens(balance2, 18))?.toFixed(3)):formatNumber(native?.displayValue)} {token2?.symbol}</span>
          </div>
          }
     ...
     {(token1.symbol==='CORE'&&token2.symbol==='WCORE' ) ?? (token1.symbol==='WCORE'&&token2.symbol==='CORE')?
     <>
    <TransactionButton
    transaction={() => {
      return prepareTransaction(wrap);
...
>
{standard && 'WRAP'} {inputToken?.symbol}
</TransactionButton>
     </> :  <TransactionButton
          transaction={() => {
